...
public class StrobeShaderEffect extends GLShaderEffect {
  ...
  public StrobeShaderEffect(LX lx) {
    ...
    addShader(
        GLShader.config(lx).withFilename("shader_strobe.fs").withUniformSource(this::setUniforms));
  }
  ...
  /**
   * Compute the raw strobe LFO for a given basis in [0..1], apply bias curve shaping, return scalar
   * in [0..1] (1 = pass-through, 0 = blackout).
   */
