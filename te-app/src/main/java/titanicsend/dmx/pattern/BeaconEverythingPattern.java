...
/**
 * A basic beacon pattern with access to all the standard controls, with the extra DMX fluff
 * removed.
 *
 * <p>Always sets shutter to Open.
 */
@LXCategory("DMX")
public class BeaconEverythingPattern extends BeaconPattern
    implements UIDeviceControls<BeaconEverythingPattern> {
  ...
  CompoundParameter strobeSpeed =
      new CompoundParameter("StrbSpd", 64, 64, 95).setDescription("Strobe speed, slow->fast");

  BooleanParameter isStrobe = new BooleanParameter("Strobe!", false).setMode(Mode.MOMENTARY);
  ...
  protected void run(double deltaMs) {
    ...
    int shutter =
        this.isStrobe.isOn() ? (int) this.strobeSpeed.getValue() : BeaconModel.SHUTTER_OPEN;
