...
public class TEUserPresetParameter extends ObjectParameter<UserPreset> {
    ...
    public TEUserPresetParameter(TEPerformancePattern pat, UserPresetCollection collection, String label) {
        super(label, collection.getPresets().toArray(new UserPreset[0]));
        this.pattern = pat;
        this.collection = collection;
        this.updateLabels();

        this.collection.addListener(new UserPresetCollection.Listener() {
            @Override
...
//        });
    ...
    protected void updateLabels() {
        String[] options = Arrays.stream(getObjects()).map(LXComponent::getLabel).toList().toArray(new String[0]);
        setOptions(options);
    }
}
