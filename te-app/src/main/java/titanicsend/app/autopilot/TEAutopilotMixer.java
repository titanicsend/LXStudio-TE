...
public class TEAutopilotMixer {
  ...
  public void turnDownAllChannels(boolean onlyAffectPhraseChannels) {
    for (TEChannelName name : TEChannelName.values()) {
      if (onlyAffectPhraseChannels
          && ((name == TEChannelName.STROBES) || name == TEChannelName.TRIGGERS)) continue;
      this.setFaderTo(name, 0.0);
    }
  }

  /**
   * Since we can't return multiple values in Java, this is a struct for doing so.
   *
  ...
  /**
   * Function to determine if AutoVJ is set up correctly. Today we run this every Autopilot loop,
   * but can optimize do every few loops, only upon channels being removed, etc.
   *
   * <p>Scans our channels to find a AutoVJ compatible group and record its index in the LX mixer if
   * so.
   *
   * <p>Also return a boolean "found" if the AutoVJ compatible group that was also valid, and
   * doesn't need to be removed, cleaned up, and replaced.
   *
    ...
    for (LXAbstractChannel ch : this.lx.engine.mixer.channels) {
      if (ch.isGroup() && ch.label.getString().equals(AUTO_VJ_GROUP_NAME)) {
        ...
        for (LXChannel vjCh : group.channels) {
          for (TEChannelName name : TEChannelName.values()) {
            // if (verbose) TE.log("Checking channel: %s against channel name %s",
            // vjCh.label.getString(), name.toString());
  ...
  /**
   * Recursively remove IDs from a JSON object. Useful when you want to load a bunch of channels and
   ...
   *
   * <p>If a compatible group is not found, create one and populate it with the AutoVJ library of
   * patterns.
   *
   * <p>Also populates: - channel2idx (mapping from channel name -> index on LX mixer) -
   * autoVjGroupIndex (index of the AutoVJ group on the LX mixer)
   *
   * @return setupWasNeeded boolean so that we know whether we need to re-index the TELibrarian for
   *     patterns on the AutoVJ channels
   */
      ...
      if (!firstScan.found) {
        ...
        for (JsonElement chan : channelsArray) {
          ...
          if (autoVjTemplateGroupId == -1) {
            if (label.equals(AUTO_VJ_GROUP_NAME)
                && classname.equals("heronarts.lx.mixer.LXGroup")) {
