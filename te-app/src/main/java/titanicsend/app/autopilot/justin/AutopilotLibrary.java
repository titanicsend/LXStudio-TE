...
public class AutopilotLibrary {
  ...
  private final Map<Class<? extends LXPattern>, AutoPattern> patternLookup =
      new HashMap<Class<? extends LXPattern>, AutoPattern>();
  ...
  public AutoPattern addPattern(Class<? extends LXPattern> pattern) {
    if (patternLookup.containsKey(pattern)) {
      throw new IllegalStateException(
          "Cannot add same pattern class to the Autopilot Library twice: " + pattern);
    }
