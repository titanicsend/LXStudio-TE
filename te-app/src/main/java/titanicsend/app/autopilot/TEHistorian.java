 ...
 *
 * <p>Beat, downbeat, phrase OSC messages are all tracked here, along with some releated logic for
 * computing BPM or manipulating this historical data.
 *
 * <p>TEAutopilot is the orchestrator, but TEHistorian is where the data is tracked and retrieved
 * from.
 */
public class TEHistorian {
  ...
  */
  ...
  public void logMasterDeckChange(long timestamp, int deckNum, int faderVal) {
    ...
    //       timestamp, System.currentTimeMillis(), lastMasterChangeAt,
    // this.calcMsSinceLastDeckChange());
  ...
  /**
   * Including the current phrase, how many times in a row have we hit this phrase?
   *
   * @return int
   */
  ...
  /**
   * Over the repeated phrases we can see (starting with current phrase), how many bars did this
   * span? Returns a fractional number of bars.
   *
   * <p>This INCLUDES current time into the phrase since it started, whereas
