 ...
 *
 * <p>This is used to tag the message with the inbound time to allow for better estimation of BPM
 * and other time-oriented calculations.
 */
public class TEOscMessage {
  ...
  /**
   * Extract the deck number: /te/mixer/fader/4 31
   *
   * <p>Would extract -> 4
   *
  ...
  /**
   * Extract the fader number: /te/mixer/fader/4 31
   *
   * <p>Would extract -> 31
   *
  ...
  /**
   * There is some weirdness with the way LX parses OSC messages that are supposed to be floats. It
   * doesn't work, and this was easier than figuring out what the issue was and patching it.
   *
   * <p>For whatever reason, BPM-related template values in the "EXECUTOR | BEAT CHANGE" field just
   * don't behave like other fields! You have to format as a string. So here we are...
   *
  ...
  public static void sendOscToPangolin(LX lx, String address, int value, boolean verbose) {
    if (verbose)
      TE.log(
          "Sending OSC to %s:%d = OSC OUT: %s %d;",
          PangolinHost.HOSTNAME, PangolinHost.PORT, address, value);
