...
public class UIDevSwitch extends UICollapsibleSection implements UIControls {
  ...
  public UI2dContainer addRow(
      UI2dContainer uiDevice, float rowHeight, UI2dComponent... components) {
  ...
  public UI2dContainer addRow(
      UI2dContainer uiDevice, float rowHeight, String label, UI2dComponent... components) {
    ...
    if (components != null) {
      float cWidth =
          (label == null
                  ? getContentWidth() - (PADDING * 2)
                  : this.controlWidth - ((components.length - 1) * CHILD_SPACING))
              / components.length;
  ...
  public UI2dComponent addHorizontalBreak(UI ui, UI2dContainer uiDevice) {
    return new UI2dComponent(0, 0, uiDevice.getContentWidth() - (PADDING * 2), 1) {}.setBorderColor(
