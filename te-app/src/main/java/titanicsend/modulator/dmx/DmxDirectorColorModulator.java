...
public class DmxDirectorColorModulator extends AbstractDmxModulator
  ...
  public final ColorParameter color =
      new ColorParameter("Color", LXColor.BLACK).setDescription("Color received by DMX");
  ...
  private UILabel label(LXStudio.UI ui, String label, float width, float x) {
    return (UILabel) controlLabel(ui, label, width).setPosition(x, 20);
  }

  UI2dComponent colorUi;

  public void buildModulatorControls(
      LXStudio.UI ui, UIModulator uiModulator, final DmxDirectorColorModulator dmx) {
    ...
    uiModulator.addChildren(
        ...
        label(ui, "Chnl", 28, 36),
        UIFixture.constructDmxTooltip()
            .setPlacement(UIButton.Tooltip.Placement.BOTTOM_LEFT)
            .setPosition(64, 19),
