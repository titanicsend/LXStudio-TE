...
/**
 * Use symmetric edges in the model, and colors them together or produces black and white mask to
 * multiply-blend other content. `edgesBySymmetryGroup` is Vector -> List<Edges>, where the vector
 * has x zeroed, and catalogs them by the abs(z). Therefore, 1, 2, or 4 edges will be in each List,
 * and they all have either port-starboard or fore-aft symmetry (or both).
 *
 * <p>Color is ignored if placed in a channel set to the multiply blendMode, and the pattern outputs
 * a white & black mask.
 */
