 ...
 *
 * <p>- Applying an aspect of the sound data (bass) to a pattern (mid-edge pulse width) - How to use
 * a parameter that allows the VJ to select the number of bass bands averaged
 *
 * <p>Play with the decay setting of the GraphicMeter in the global UI.
 *
 * <p>TEAudioPattern provides bassLevel and other fields and methods for processing the live audio
 ...
 * click "GLOBAL", enable the Audio with the green top-left toggle, and select an input. To route
 * music to your speakers AND to LX at the same time, use BlackHole. BlackHole can be useful to get
 * the songs you play on your computer to loopback as an audio input (simulating the line-in audio
 * feed we’ll have on the car). Use Audio Midi Setup to configure a multi-output device:
 *
 * <p>https://existential.audio/blackhole/
  ...
  public void runTEAudioPattern(double deltaMs) {
    ...
    for (TEEdgeModel edge : this.modelTE.getEdges()) {
      ...
      for (TEEdgeModel.Point point : edge.edgePoints) {
        ...
        if (point.n >= lowFrac && point.n < highFrac) colors[point.point.index] = LXColor.RED;
      }
    }
  }
}
