...
@LXCategory("Combo FG")
public class BassParticles extends GLShaderPattern {
  private TEShader renderShader;

  // --- Multi-Ball 2D Demo ---
  private static final int MAX_SPHERES = 200; // 200 balls in 2D plane
  ...
  // Gravity centers (attractors) at z = 1.5 forming an upright triangle in XY space
  private final java.util.List<javax.vecmath.Vector3f> gravityCenters = new java.util.ArrayList<>();
  private float baseGravityStrength = 25.0f; // Tunable base strength
  private float gravityFalloffEpsilon = 0.25f; // Prevent singularities (in world units^2)
  private float maxForcePerBody = 50.0f; // Clamp per-body force each frame
  ...
  private void setCustomUniforms(GLShader shader) {
    if (shader == this.renderShader) {
      ...
      // Add depth scale factor for Z-based perspective (keep subtle as z is fixed)
      shader.setUniform("depthScaleFactor", 1.2f);
  ...
  private void updateCurrentTransforms() {
    ...
    for (ActiveSphere sphere : activeSpheres) {
      ...
      if (shouldLog()) {
        ...
        TE.log(
            "BassParticles: Sphere '%s' - World(%.2f, %.2f, %.2f) Norm(%.3f, %.3f, %.3f) Radius=%.3f NormRadius=%.3f LogFPS=%.1f",
            sphere.name,
            pos[0],
            pos[1],
            pos[2],
  ...
  private void updatePhysicsPositions() {
    ...
    if (shouldLogNow) {
      TE.log("BassParticles: updatePhysicsPositions - Processing %d spheres", activeSpheres.size());
    }

    physicsPositionBuffer.clear();
    physicsSizeBuffer.clear();
    float sizeParam = (float) controls.getLXControl(TEControlTag.SIZE).getValuef();
    float bassEnv = (float) Math.max(0.0, Math.min(1.0, this.bassLevel));
    float audioSizeScale = 1.0f + 0.8f * bassEnv; // breathe with bass
    for (int i = 0; i < activeSpheres.size(); i++) {
      ...
      float normalizedRadius = as.radius * sizeParam * audioSizeScale * WORLD_TO_NORMALIZED_SCALE;
      physicsSizeBuffer.put(normalizedRadius);

      // Log first sphere details at controlled FPS rate
      if (i == 0 && shouldLogNow) {
        TE.log(
            "BassParticles: Sphere 0 '%s' - Shader gets Norm(%.3f, %.3f, %.3f) NormRadius=%.3f",
  ...
  private void initializeBulletPhysics() {
    try {
      TE.log("BassParticles: Initializing 2D multi-ball physics demo with SceneConfig...");

      // Ball radius (will be used to calculate Z room thickness)
      float ballRadius = 0.04f; // 2x larger than previous
      float zThickness = 1.5f * (2.0f * ballRadius); // 1.5 times the ball diameter
      float zCenter = 0.0f; // Z layer at 0.0 per spec
      ...
      SceneConfig config =
          new SceneConfig()
              .withGlobalGravity(new javax.vecmath.Vector3f(0f, 0f, 0f)) // Zero global gravity
              .withRoom(
      ...
      // Spawn balls evenly across XY plane at z = 1.5
      spawnBallsOnGrid();

      // Set up three gravity centers forming a larger upright triangle in XY at z = 0.0
      gravityCenters.clear();
      gravityCenters.add(new javax.vecmath.Vector3f(1.5f, 1.5f, 0.0f)); // Bottom left
      gravityCenters.add(new javax.vecmath.Vector3f(8.5f, 1.5f, 0.0f)); // Bottom right
      gravityCenters.add(new javax.vecmath.Vector3f(5.0f, 8.5f, 0.0f)); // Top center

      bulletPhysicsInitialized = true;
      TE.log("BassParticles: 2D multi-ball physics demo initialized with SceneConfig");

    } catch (Exception e) {
      TE.error(e, "BassParticles: Failed to initialize Bullet Physics");
      bulletPhysicsInitialized = false;
    }
  }

  private void spawnBallsOnGrid() {
    int numBalls = Math.min(MAX_SPHERES, 200);
    float ballRadius = 0.04f; // 2x larger than previous
    float zFixed = 0.0f; // Fixed z layer at 0.0

    // Grid layout: 20 x 10
    int cols = 20;
    int rows = (int) Math.ceil(numBalls / (float) cols);

    float minXY = 1.0f;
    float maxXY = 9.0f;
    float xStep = (maxXY - minXY) / (cols - 1);
    float yStep = rows > 1 ? (maxXY - minXY) / (rows - 1) : 0f;

    int count = 0;
    for (int r = 0; r < rows; r++) {
      for (int c = 0; c < cols; c++) {
        if (count >= numBalls) break;
        float x = minXY + c * xStep + (float) (Math.random() - 0.5) * 0.05f;
        float y = minXY + r * yStep + (float) (Math.random() - 0.5) * 0.05f;
        float z = zFixed;
        ...
        if (body != null) {
          ...
          TE.log("BassParticles: Spawned sphere '%s' at World(%.2f, %.2f, %.2f)", name, x, y, z);
        }
    ...
    TE.log("BassParticles: Spawned %d spheres on grid", activeSpheres.size());
  }
  ...
  private void tickBulletPhysics(float dtSeconds) {
    ...
    try {
      ...
      // Apply attraction forces toward gravity centers (audio-reactive)
      float bassEnv = (float) Math.max(0.0, Math.min(1.0, this.bassLevel));
      float gravityMult = 0.7f + 0.6f * bassEnv;
      for (ActiveSphere sphere : activeSpheres) {
        RigidBody body = bulletPhysics.getBody(sphere.name);
        if (body == null) continue;
        float[] pos = bulletPhysics.getBodyPosition(sphere.name);
        javax.vecmath.Vector3f netForce = new javax.vecmath.Vector3f(0f, 0f, 0f);
        for (javax.vecmath.Vector3f center : gravityCenters) {
          javax.vecmath.Vector3f dir =
              new javax.vecmath.Vector3f(center.x - pos[0], center.y - pos[1], 0f);
          float d2 = dir.x * dir.x + dir.y * dir.y + gravityFalloffEpsilon;
          float invLen = (float) (1.0 / Math.sqrt(Math.max(d2, 1e-6)));
          dir.scale(invLen);
          float strength = baseGravityStrength * gravityMult / d2;
          dir.scale(strength);
          netForce.add(dir);
        }
        // Clamp force
        float len = (float) Math.sqrt(netForce.x * netForce.x + netForce.y * netForce.y);
        if (len > maxForcePerBody) {
          netForce.scale(maxForcePerBody / len);
        }
        body.applyCentralForce(netForce);
        body.activate(true);
      }
      ...
      if (timeSinceLastKick >= kickInterval && !activeSpheres.isEmpty()) {
        ...
        if (body != null) {
          ...
          float mag = 2.5f + (float) Math.random() * 6.0f;
      ...
      TE.error(e, "BassParticles: Error in Bullet Physics simulation");
    }
  }
  ...
  protected void onWowTrigger(boolean on) {
    ...
    if (on && bulletPhysicsInitialized && !activeSpheres.isEmpty()) {
      ...
      TE.log(
          "BassParticles: WOW TRIGGER! 2D wave affecting %d spheres with strength %.1f",
      ...
      for (ActiveSphere sphere : activeSpheres) {
        ...
        if (body != null) {
          ...
          TE.log(
              "BassParticles: Applying 2D impulse (%.2f, %.2f) to sphere '%s'",
