...
@LXCategory("Combo FG")
public class ParticleTest3D extends GLShaderPattern {
  ...
  private void initializeBulletPhysics() {
    try {
      TE.log("ParticleTest3D: Initializing 3D multi-ball physics demo with SceneConfig...");
      ...
      TE.log("ParticleTest3D: 3D multi-ball physics demo initialized with SceneConfig");

    } catch (Exception e) {
      TE.error(e, "ParticleTest3D: Failed to initialize Bullet Physics");
  ...
  private void tickBulletPhysics(float dtSeconds) {
    ...
    try {
      ...
      if (!spawnedStaticSet) {
        ...
        float minX = 0.8f, maxX = 9.2f;
        float minY = 0.8f, maxY = 9.2f;
        float minZ = 0.8f, maxZ = 9.2f;

        for (int i = 0; i < numBalls && i < MAX_SPHERES; i++) {
          int row = i / gridSize, col = i % gridSize;
          float fx = col / (float) Math.max(1, gridSize - 1);
          float fy = row / (float) Math.max(1, gridSize - 1);

          float x = minX + fx * (maxX - minX);
          float y = minY + fy * (maxY - minY);
          float z = minZ + (float) Math.random() * (maxZ - minZ);

          float radiusVariation = 0.2f + (float) Math.random() * 0.4f;
          float ballRadius = 0.4f * radiusVariation;

          String name = "sphere-" + (spawnCounter++);
          bulletPhysics.addSphere(name, x, y, z, ballRadius, sphereMass * radiusVariation, true);
          ...
          if (body != null) {
            ...
            TE.log("ParticleTest3D: Spawned sphere '%s' at World(%.2f, %.2f, %.2f)", name, x, y, z);
          }

          // (these helpers must exist in BulletBootstrap)
