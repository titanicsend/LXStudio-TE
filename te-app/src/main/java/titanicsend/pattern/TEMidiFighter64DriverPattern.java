...
public class TEMidiFighter64DriverPattern extends TEPattern implements LXMidiListener {
  ...
  private static final int[] buttonColors = {
    ...
    LED_RED_DIM, LED_ORANGE_DIM, LED_YELLOW_DIM, LED_GREEN_DIM, LED_BLUE_AQUA_DIM, LED_AZURE_DIM,
        LED_MAGENTA_DIM, LED_GRAY_DIM,
    LED_RED, LED_ORANGE, LED_YELLOW, LED_GREEN, LED_BLUE_AQUA, LED_AZURE, LED_MAGENTA, LED_WHITE,
    LED_RED_DIM, LED_ORANGE_DIM, LED_YELLOW_DIM, LED_GREEN_DIM, LED_BLUE_AQUA_DIM, LED_AZURE_DIM,
        LED_MAGENTA_DIM, LED_GRAY_DIM,
    LED_RED, LED_ORANGE, LED_YELLOW, LED_GREEN, LED_BLUE_AQUA, LED_AZURE, LED_MAGENTA, LED_WHITE,
  ...
  private final TEMidiFighter64Subpattern[] buttonAssignments = {
    ...
    randomPanel, randomPanel, randomPanel, randomPanel, randomPanel, randomPanel, randomPanel,
        randomPanel,
  ...
  public final DiscreteParameter rowShift1 =
      new DiscreteParameter("Row1", 0, 8).setDescription("Row1 Offset");
  public final DiscreteParameter rowShift2 =
      new DiscreteParameter("Row2", 0, 8).setDescription("Row2 Offset");

  public final DiscreteParameter pokeVelocity =
      new DiscreteParameter("Vel", 0, 128).setDescription("Velocity");

  public final DiscreteParameter pokePitch =
      new DiscreteParameter("Pitch", 0, 128).setDescription("Pitch");
  ...
  private final LXParameterListener fakepushListener =
      (p) -> {
  ...
  private final LXParameterListener pokeListener =
      (p) -> {
        if (this.pokeButton.isOn()) {
          if (this.midiOut == null) {
            ...
            LX.log(
                "Poke MF64 at Channel="
                    + this.pokeChannel.getValuei()
                    + ", Pitch="
                    + this.pokePitch.getValuei()
                    + ", Velocity="
  ...
  /**
   * Given the location (row,column,page) of a button, and a color, in LX packed color format, set
   * the color of the button (sets the nearest available color the button can produce. YMMV)
   *
   * @param row
   * @param column
   * @param page
  ...
  private final LXParameterListener midiOutConnectedListener =
      (p) -> {
