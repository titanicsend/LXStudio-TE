...
public abstract class TEMidiFighter64Subpattern {
  ...
  */
  ...
  /**
   * Set pixel to specified color at full opacity. Assumes that the color value already has the
   * desired brightness.
   */
  ...
  /**
   * Set pixel color at index with controlled "blending". Note that is fast rather than accurate
   * blending. It simply takes the color with the highest alpha value.
   */
  ...
  /**
   * Set pixel color only if underlying color hasn't previously been set (or has been deliberately
   * cleared). This is used to let patterns like "flash" work as a colored backdrop for more complex
   * patterns
   */
