...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    for (float j = 0; j < v; j++) {
        ...
        t += TAU * .618;
        // and its own color (higher iWow1 == more monochromeness)
        starColor = mix(getGradientColor(j / v),iColorRGB,iWow1);
        for (float i = 1.; i <= v; i++) {
            float pct = 0.5 * step (0.5, i/v);
            // shape the beat sawtooth so its peak will be very short
            float pulse = 1.0 - fract(beat + pct);
            pulse = levelReact * pulse * pulse;
            ...
            float beatBlink = (abs(rij) < 0.2) ? 1.0 + levelReact * (1.0 - wave(4.0 * pulse)) : 1.0;
