...
We use [LX Studio](https://lx.studio/) to control the light show, and this README will get you up and running with LX
Studio so you, too, can help us create some great patterns.
...
> These are geared toward running LX Studio on a Macbook with `git` already installed. If you need help with anything,
> ask in the Slack #lighting-software channel!
...
We're using [CheckStyle](https://github.com/checkstyle/checkstyle) to store our java formatting rules (since
it's editor-independent, has plugins for IntelliJ, Eclipse, and Maven). We base our formatting rules off of
the [Google Java Style Guide](https://google.github.io/styleguide/javaguide.html), for which
checkstyle [provides an XML file](https://checkstyle.org/google_style.html). Our config file is stored in this repo at
...
1. **Install the Checkstyle plugin:**
    ...
    2. Install [Checkstyle-IDEA](https://plugins.jetbrains.com/plugin/1065-checkstyle-idea).
2. **Enable Checkstyle code inspections within the IDE:**
    1. Go to `Settings > Tools > Checkstyle`, and click the `+`
       button
       above configuration files.
    2. Choose `checkstyle.xml` from the root
       of the repo and title it `TE Checkstyle XML`.
    3. Click `Apply` at the bottom of the settings window.
3. **Enable code formatting using checkstyle rules within the IDE:**
    1. Go to `Settings > Editor > Code Style > Java`.
    2. Under the dropdown menu labeled `Scheme`, select `Project`.
    3. Click the gear icon next to that dropdown, choose `Import Scheme > Checkstyle Configuration`
    4. Use the file picker to find `checkstyle.xml`.
    5. Click `Apply` at the bottom of the settings window.
    6. _(Note: if `Checkstyle Configuration` is not an option under `Import Scheme`, you may need to restart IntelliJ if
       you haven't restarted since installing the Checkstyle plugin)._
4. (Optional) **Enable auto formatting on save:**
    1. Go to `Settings > Tools > Actions on Save`.
    2. Check the boxes for `Reformat Code`, `Optimize Imports`, and `Run Code Cleanup`.
    3. Click `Apply` at the bottom of the settings window.

#### Eclipse Code Formatting

1. Install [Eclipse Checkstyle Plugin](https://checkstyle.org/eclipse-cs/#!/)
2. Import `checkstyle.xml` "stylesheet" using `Preferences > General > Checkstyle > New`.
3. Since you have an external file, choose "external file" as the type and find `checkstyle.xml`
4. Right-click on your project in the Package view, and select `Checkstyle > Create Formatter-Profile`.
5. Enable the formatter for your workspace: Under `Preferences > Java > Code Style > Formatter`, select
   formatter: `eclipse-cs [project name]`.
