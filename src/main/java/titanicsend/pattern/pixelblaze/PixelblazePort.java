...
public abstract class PixelblazePort extends TEPerformancePattern {
  ...
  private LXPoint[] getModelPoints() {
    ...
    if (enableEdges.getValueb()) {
      newPoints.addAll(this.modelTE.getEdgePoints());
    }
    if (enablePanels.getValueb()) {
      newPoints.addAll(this.modelTE.getPanelPoints());
    }
  ...
  /**
   * Called before adding common controls. Derived classes can override this to change control
   * ranges, response curves and other parameters if necessary.
   *
   * <p>If a pattern needs to add additional custom controls, it can do so by implementing them in
   * setup().
   */
