...
public class TEGradientPattern extends TEPerformancePattern {
    ...
    public static enum CoordinateMode {
        ...
        private CoordinateMode(String name, CoordinateFunction function) {
            ...
            this.invert = (p, normalized, offset) -> {
    ...
    public final EnumParameter<CoordinateMode> xMode = new EnumParameter<CoordinateMode>(
                    "X Mode", CoordinateMode.NORMAL)
            .setDescription("Which coordinate mode the X-dimension uses");

    public final EnumParameter<CoordinateMode> yMode = new EnumParameter<CoordinateMode>(
                    "Y Mode", CoordinateMode.NORMAL)
            .setDescription("Which coordinate mode the Y-dimension uses");

    public final EnumParameter<CoordinateMode> zMode = new EnumParameter<CoordinateMode>(
                    "Z Mode", CoordinateMode.NORMAL)
    ...
    protected void runTEAudioPattern(double deltaMs) {
        ...
        for (LXPoint p : this.model.points) {
            ...
            final float xn = p.xn * this.transform.m11
                    + p.yn * this.transform.m12
                    + p.zn * this.transform.m13
                    + this.transform.m14;

            final float yn = p.xn * this.transform.m21
                    + p.yn * this.transform.m22
                    + p.zn * this.transform.m23
                    + this.transform.m24;

            final float zn = p.xn * this.transform.m31
