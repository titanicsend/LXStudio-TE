...
public class ModelDebugger extends TEPattern implements UIDeviceControls<ModelDebugger> {
    ...
    public final EnumParameter<ObjectType> objectType = new EnumParameter<ObjectType>("Object Type", ObjectType.PANEL)
            .setDescription("Which type of object to light up");

    public final DiscreteParameter pointIndex = new DiscreteParameter("Index", -1, 2000)
    ...
    public void repaint(LXParameter unused) {
        ...
        switch (this.objectType.getEnum()) {
            ...
            case EDGE:
                ...
                else if (this.modelTE.edgesById.containsKey(idStr)) subModels.add(this.modelTE.edgesById.get(idStr));
                else this.idErrLabel.setVisible(true);
                break;
            case PANEL:
                if (getAll) subModels.addAll(this.modelTE.panelsById.values());
                else if (this.modelTE.panelsById.containsKey(idStr)) subModels.add(this.modelTE.panelsById.get(idStr));
                else this.idErrLabel.setVisible(true);
                break;
            case LASER:
                // TODO: Implement
