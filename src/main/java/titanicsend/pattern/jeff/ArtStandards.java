 ...
 *
 * <p>https://docs.google.com/document/d/16FGnQ8jopCGwQ0qZizqILt3KYiLo0LPYkDYtnYzY7gI/edit
 *
 * <p>- Tempo reactive (the colors flow out sync'd to every 4 beats) - Sound reactive, sparkling
 * white when there's lots of treble - Uses the global palette, a simple gradient from the edge
 * foreground to the secondary color - Uses the energy parameter we want all TEPatterns to consume -
 * Respects the geometric form - it's volumetric based on the distance from the center of the car;
 * not a screen projection
 *
 * <p>It is preloaded and intended to be used from within the Audio Examples workspace project
 * (Select this at the top center of the UI).
 */
...
public class ArtStandards extends TEAudioPattern {
  ...
  public void runTEAudioPattern(double deltaMs) {
     ...
     */
    double scaledTrebleRatio =
        LXUtils.clamp(
            (trebleRatio - .5) / (1.01 - energy.getNormalized()) / 6
                - .2
                + energy.getNormalized() / 2,
            0,
            1);
