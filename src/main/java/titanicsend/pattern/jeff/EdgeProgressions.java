 ...
 *
 *  The scene list is a sequence that can repeat scenes. The pattern steps
 *  through them on a tempoDivision (such as every beat or measure), or
 *  when triggered by audio, or via manual scene selection. In Manual
 *  mode, the current scene can be advanced by a trigger (which can be
 *  linked to a MIDI note or triggered by the beat modulator).
 *
 *  By default, loading an edgeSet will also include all symmetric edges
 *  found in the model, this can be overridden in the JSON file with
 *  `"includeSymmetric": false`.
 *
...
public class EdgeProgressions extends TEAudioPattern {
    ...
    public void runTEAudioPattern(double deltaMs) {
        ...
        // TODO Swap when fix https://github.com/heronarts/LX/commit/c10fba3a15766bd9441506a09e3e27e938f9b4e0
        // is in our snapshot version of 0.4.1.
        // if (triggerMode.getEnum().equals(Mode.TEMPO) && tempoDivisionClick.click()) nextScene();
        if (triggerMode.getEnum().equals(Mode.TEMPO) && tempoDivisionClick.getBasis() < lastClickBasis) nextScene();
    ...
    protected void loadScenes() {
        ...
        for (EdgeSet es : edgeSetData) {
            ...
            for (String edgeId : es.edgeIds) {
                if (model.edgesById.containsKey(edgeId)) {
                    TEEdgeModel edge = model.edgesById.get(edgeId);
