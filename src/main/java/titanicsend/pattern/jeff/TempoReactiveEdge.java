...
public class TempoReactiveEdge extends TEPattern {
    ...
    public void run(double deltaMs) {
        ...
        // (could also do lx.engine.tempo.beatCount() % 4)
        ...
        // Max width of the traveling pulse, expressed as a percentage of this edge, up to 37.5%
        double pulseWidthPct = energy.getNormalized() * 3 / 8;
        ...
        */
        ...
        // position from 0 to (100% + pulseWidthPct).
        // Instructional - not used. We'll modify this in a second.
        double pulseHeadPct = measure * (1 + pulseWidthPct);

        // Now, pulse's head position WITH quarter note swing, still
        // from 0 to (100% + pulseWidthPct).
        // Compare to pulseHeadPct above.
        pulseHeadPct = (beatInMeasure + swingBeat) / 4 * (1 + pulseWidthPct);

        for (TEEdgeModel edge : model.edgesById.values()) {
            double pulseTailPct = pulseHeadPct - pulseWidthPct;

            for (TEEdgeModel.Point point : edge.points) {
                // point.pct is the 0..1 percentage this point is into its edge
                if (point.pct >= pulseTailPct && point.pct < pulseHeadPct)
