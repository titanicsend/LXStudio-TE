...
// TODO if there are multiple controllers at a single vertex, we are not differentiating as this
// info is not yet
...
public class SignalDebugger extends TEPattern implements UIDeviceControls<SignalDebugger> {
  ...
  public final DiscreteParameter vertexSelectParameter =
      new DiscreteParameter("Controller Vertex", 0, 150)
          .setDescription(
              "Show all signal routes stemming from the controller at the designated vertex id");
  ...
  public final StringParameter edgeIdParameter =
      new StringParameter("EdgeId")
          .setDescription("Show signal route that edge of designated id belongs to");

  private static final List<Integer> CHANNEL_COLORS =
      List.of(
  ...
  private final Map<Integer, List<List<ChainedEdge>>> controllerVertexToEdgeRoutes =
      new HashMap<>();
  private List<Integer> allControllers;
  ...
  public void run(double deltaMs) {
    ...
    if (activePanelIds != null) {
      for (String activePanelId : activePanelIds) {
        int routeColor = colorIter.next();
        List<LXPoint> activePoints = modelTE.getPanel(activePanelId).getPoints();
  ...
  private void loadPanelData() {
    ...
    while (s.hasNextLine()) {
      ...
      if (type.equals("solid")
          || (!showLowPriParameter.getValueb() && priority > LOWEST_PANEL_PRIORITY)) {
        continue;
      }
  ...
  public void onParameterChanged(LXParameter parameter) {
    ...
    if (parameter.equals(vertexSelectParameter)) {
      ...
      activePanelIds = controllerVertexToPanelIds.get(controllerVertex);
      vertexSelectParameter.setValue(controllerVertex);
