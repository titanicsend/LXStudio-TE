...
public class TECommonControls {
  ...
  public final BooleanParameter panic = new BooleanParameter("PANIC", false)
  ...
  public void buildDefaultControlList() {
    ...
    p = new CompoundParameter(TEControlTag.SPIN.getLabel(), 0, -1.0, 1.0)
        .setPolarity(LXParameter.Polarity.BIPOLAR)
        .setNormalizationCurve(BoundedParameter.NormalizationCurve.BIAS_CENTER)
        .setExponent(2)
        .setDescription("Spin");
    ...
    p = (LXListenableNormalizedParameter)
        new TECommonAngleParameter(this.pattern, this.pattern.spinRotor,
            TEControlTag.ANGLE.getLabel(), 0, -Math.PI, Math.PI)
  ...
  public TECommonControls setNormalizationCurve(TEControlTag tag,
  ...
  private static LXListenableNormalizedParameter updateParam(
      LXListenableNormalizedParameter oldControl, String label, double value, double v0,
      double v1) {
    LXListenableNormalizedParameter newControl;
    if (oldControl instanceof CompoundParameter) {
      newControl = new CompoundParameter(label, value, v0, v1)
      ...
      newControl = new BoundedParameter(label, value, v0, v1)
      ...
      newControl = new BooleanParameter(label)
          .setMode(((BooleanParameter) oldControl).getMode())
          .setDescription(oldControl.getDescription())
          .setUnits(oldControl.getUnits());
    } else if (oldControl instanceof DiscreteParameter) {
      newControl = new DiscreteParameter(label,
          ((DiscreteParameter) oldControl).getOptions())
  ...
  protected void setRemoteControls() {
    this.pattern.setCustomRemoteControls(this.color.offset,
        this.color.gradient,
