...
public class RhythmicFlashingStatic extends FragmentShaderEffect {
  ...
  private double cell(double[] c) {
    ...
    //        return (3.0 - length(uv)) * step(fract( sin(c.x + c.y*50.0) *1000.0), 0.04); // 3.0 ::
    // star width and fade strength, 0.04 :: star count
    double cx = newC[0];
    double cy = newC[1];
  ...
  protected double[] getColorForPoint(
    ...
    double[] projection =
        new double[] {fragCoordinates[0] / RESOLUTION, fragCoordinates[1] / RESOLUTION};
    ...
    //            vec2 coord = vec2(projY, projX) * 256.; // *512 :: star count, adjust star-count
    // and scale for scaling along Y axis
    double[] coord = new double[] {projX * STAR_COUNT_INT, projY * STAR_COUNT_INT};

    //            vec2 delta = vec2(time * 16.0, 0); // time*7.0 :: speed2 of stars, adjust both^^
    // speeds for length of stars
    ...
    double brightness =
        (1.0 - color) * (1.0 - (random() * this.getTempo().basis() * pattern.getWow1()));
