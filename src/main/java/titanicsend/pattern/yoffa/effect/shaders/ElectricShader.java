...
public class ElectricShader extends FragmentShaderEffect {
    ...
    private double[] hash(double[] p) {
        p = new double[] {dotProduct(p, new double[] {127.1, 311.7}), dotProduct(p, new double[] {269.5, 183.3})};
        return addToArray(
                -1,
                multiplyArray(
                        dispersion.getValue(),
                        fract(Arrays.stream(p)
                                .map(v -> Math.sin(v) * 43758.5453123)
                                .toArray())));
    }
    ...
    private double noise(double[] p) {
        ...
        double[] nTmp = {
            dotProduct(a, hash(i)), dotProduct(b, hash(addArrays(i, o))), dotProduct(c, hash(addToArray(1, i)))
        };
