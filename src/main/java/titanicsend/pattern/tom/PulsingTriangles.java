...
public class PulsingTriangles extends TEPattern {
    ...
    public final BooleanParameter tempoLock = new BooleanParameter("Lock", true)
            .setDescription("Whether this modulator is locked to the beat grid or free-running");

    protected final CompoundParameter rate = (CompoundParameter) new CompoundParameter("Rate", .50, .01, 2)
            .setExponent(2)
    ...
    private HashMap<String, LXPoint[][]> buildPointMap(HashMap<String, TEPanelModel> panels) {
        return panels.entrySet().stream()
                .collect(
                        Collectors.toMap(e -> e.getKey(), e -> buildPanelMap(e.getValue()), (a, b) -> a, HashMap::new));
    }
    ...
    private static double distanceBetweenPointAndLineSegment(LXVector line_p1, LXVector line_p2, LXPoint p) {
        ...
        double t =
                Math.max(0, Math.min(1, new LXVector(v0).sub(v1).dot(new LXVector(v2).sub(v1)) / segmentLengthSquared));
