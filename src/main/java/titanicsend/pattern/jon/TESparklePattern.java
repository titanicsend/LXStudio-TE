 ...
 *     <p>This version: adapted for use with Titanic's End live performance controls - 2023
 */
...
public class TESparklePattern extends TEPerformancePattern {
    ...
    private class Sparkle {
      ...
      private void reindex(LXModel model) {
        ...
        for (int i = 0; i < this.indexBuffer.length; ++i) {
          this.indexBuffer[i] =
              LXUtils.constrain((int) (Math.random() * model.size), 0, model.size - 1);
        }
      }
    }
  ...
  public TESparklePattern(LX lx) {
    ...
    // Sparkle density
    controls
        .setRange(TEControlTag.QUANTITY, 50, 0, 100 * Engine.MAX_DENSITY)
