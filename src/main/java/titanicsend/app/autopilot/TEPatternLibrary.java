...
public class TEPatternLibrary {
  ...
  public enum TEPatternCoverageType {
    ...
    /**
     * Adjusts which types of patterns (with respect to car coverage) can come after others.
     *
     * @param coverage
  ...
  /** For patterns we catalog, can they conform to the current palette settings? Or not? */
  public enum TEPatternColorCategoryType {
    ...
    /**
     * Adjusts which types of patterns (with respect to color type) can come after others.
     *
     * @param colorCat
     * @return List<TEPatternColorCategoryType>
     */
    public static HashSet<TEPatternColorCategoryType> getCompatible(
        TEPatternColorCategoryType colorCat) {
      if (colorCat == PALETTE) {
   ...
   *
   * <p>"class titanicsend.pattern.pixelblaze.PBXorcery" or "class
   * titanicsend.pattern.yoffa.config.ShaderPanelsPatternConfig$Marbling"
   *
   * <p>Extract "PBXorcery" or "Marbling"
   *
   * @param classToString
   * @return
   */
  ...
  public ArrayList<LXPattern> getCompatibleNextPatterns(
      TEPhrase oldPhrase, LXPattern curPattern, TEPhrase newPhrase) throws Exception {
    if (!this.isReady())
      throw new Exception("Cannot filter patterns, you need to call indexPatterns() first!");
    ...
    HashSet<TEPatternCoverageType> compatibleCoverage =
        TEPatternCoverageType.getCompatible(rec.coverageType);
    ...
    if (newPhrase != TEPhrase.CHORUS) {
      ...
      // we don't fade out from the DOWN/UP before, so we're not worried about clashing, and
      // generally
      // we like want this to be a bolder transition
      s =
          s.filter(r -> compatibleCoverage.contains(r.coverageType))
    ...
    // TE.log("Matches for oldPhrase=%s, oldPattern=%s, newPhrase=%s", oldPhrase, curPattern,
    // newPhrase);
    for (TEPatternRecord r : matchingRecords) {
  ...
  /**
   * Given the currently playing pattern (on a channel determined by the current phrase type) as
   * well as the next phrase, pick a compatible next pattern to start fading into on the new
   * phrase's channel.
   *
   * <p>Considerations include phrase, coloring, coverage, and play history. Of these, phrase is the
   * most important, as not every pattern is on every channel -- we must filter there.
   *
   * @param curPattern
   * @param curPhrase
   * @param nextPhrase
   ...
   * @throws Exception
   */
  public LXPattern pickRandomCompatibleNextPattern(
      LXPattern curPattern, TEPhrase curPhrase, TEPhrase nextPhrase) throws Exception {
    ...
    // filter patterns
    ArrayList<LXPattern> matchingPatterns =
        getCompatibleNextPatterns(curPhrase, curPattern, nextPhrase);
    ...
    if (rand.nextFloat() <= PROB_PICK_RANDOM_NEXT_PATTERN) {
      ...
      Collections.sort(
          ...
          new Comparator<LXPattern>() {
            ...
            public int compare(LXPattern a, LXPattern b) {
              double barsA =
                  patternHistoryCounter.get(a) == null ? 0 : patternHistoryCounter.get(a);
              double barsB =
                  patternHistoryCounter.get(b) == null ? 0 : patternHistoryCounter.get(b);
      ...
      // TE.log("After sort, first item in matchingPatterns list has %f plays, and last has %f
      // plays"
    ...
    // TE.log("Picked next pattern for [%s]: %s (index=%d)", nextPhrase, selectedPattern,
    // patternIndex);
   ...
   *
   * <p>Library has a list of TEPatternRecord, each of which need to be mapped to one (or more!)
   * LXPattern objects on an LXChannel.
   *
   * <p>Each TEPatternRecord should only map to LXPatterns on the same LXChannel.
   */
  public void indexPatterns(TEAutopilotMixer autoMixer) {
    ...
    for (TEPatternRecord r : this.patternRecords) {
      ...
      for (LXPattern p : ch.getPatterns()) {
        if (r.patternClass == p.getClass()) {
          ...
          // TE.log("[TEPatternLibrary] Index pattern=%s to channel=%s, record=%s", p.getLabel(),
          // name, r);
          found++;
        }
      }
      ...
      if (found == 0) {
        TE.err(
            "No LXPattern found for %s (channel=%s). Either add to AUTO_VJ_TEMPLATE.lxp, or remove"
                + " from TEApp::initializePatternLibrary()",
            r, name);
    ...
    if (totalFound > 0) {
      ...
      for (Map.Entry<TEPhrase, Integer> entry : patternsPerPhrase.entrySet()) {
        ...
        TE.log(
            "\tChannel=%s has %d indexed patterns",
            TEChannelName.getChannelNameFromPhraseType(pt), count);
      }
    }
    ...
    if (totalNotFound > 0) {
      ...
      for (Map.Entry<TEPhrase, Integer> entry : missingPatternsPerPhrase.entrySet()) {
        ...
        int count = entry.getValue();
        TE.log(
            "\tChannel=%s has %d missing patterns",
            TEChannelName.getChannelNameFromPhraseType(pt), count);
      }
    }
