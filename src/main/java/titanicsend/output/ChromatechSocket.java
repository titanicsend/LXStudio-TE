...
public class ChromatechSocket implements Comparable<ChromatechSocket> {
    ...
    public void addPanel(TEPanelModel panel, int firstPanelPixel, int lastPanelPixel) {
        ...
        if (this.panel != null) {
            throw new IllegalArgumentException(
                    this.repr() + " already has " + this.panel.repr() + " mapped; " + "can't also map " + panel.repr());
        }
        if (!this.edgeLinks.isEmpty()) {
            throw new IllegalArgumentException(this.repr() + " already has "
                    + this.edgeLinks.get(0).edge.repr() + " mapped; can't also map " + panel.repr());
        }
        this.panel = panel;
    ...
    public void addEdge(TEEdgeModel edge, int strandOffset, boolean fwd) {
        ...
        if (this.panel != null) {
            throw new IllegalArgumentException(
                    this.repr() + " already has " + this.panel.repr() + " mapped; " + "can't also map " + edge.repr());
        }
        ...
        for (EdgeLink edgeLink : this.edgeLinks) {
            ...
            if (existingFirstPixel <= noobLastPixel && existingLastPixel >= noobFirstPixel) {
                conflict = true;
                TE.err(edge.repr() + ", running from " + noobFirstPixel
                        + "-" + noobLastPixel + ", overlaps " + edgeLink.edge.repr()
                        + ", running from " + existingFirstPixel + "-"
                        + existingLastPixel);
            }
        }
        if (!conflict) {
    ...
    private void registerUniverses(LX lx, List<Integer> multiUniverseIndexBuffer) {
        ...
        for (int segment = 0; segment <= 2; segment++) {
            ...
            int[] ib = multiUniverseIndexBuffer.subList(first, lastPlusOne).stream()
                    .mapToInt(i -> i)
                    .toArray();
    ...
    public void activate(LX lx, int gapPointIndex) {
        ...
        StringBuilder logString = new StringBuilder("ArtNet " + this.ip.getHostAddress() + "#" + this.channelNum);
        ...
        if (isPanel && !isEdgeLinks) {
            ...
            logString.append(" " + this.panel.repr() + "[" + this.firstPanelPixel + "-" + this.lastPanelPixel + "]");
            ...
            for (EdgeLink edgeLink : this.edgeLinks) {
                ...
                logString.append(" @" + edgeLink.strandOffset + ":[" + edgeLink.edge.repr() + rStr + "="
