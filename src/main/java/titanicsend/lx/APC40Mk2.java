...
public class APC40Mk2 extends LXMidiSurface implements LXMidiSurface.Bidirectional {
    ...
    private static final Map<UserButton, BooleanParameter> userButtons = new HashMap<UserButton, BooleanParameter>();
    private static final Map<BooleanParameter, UserButton> userButtonsRev = new HashMap<BooleanParameter, UserButton>();
    ...
    private LXListenableNormalizedParameter getActiveSubparameter(AggregateParameter agg) {
        ...
        LX.error("APC40Mk2 found AggregateParameter type with no subparameter: "
                + agg.getClass().getName());
        return null;
    }
        ...
        private void registerDeviceKnobs() {
            ...
            for (LXListenableNormalizedParameter parameter : this.device.getRemoteControls()) {
                ...
                if (knobParam == null) {
                    ...
                    int ledStyle = (parameter.getPolarity() == LXParameter.Polarity.BIPOLAR)
                            ? LED_STYLE_BIPOLAR
                            : LED_STYLE_UNIPOLAR;
        ...
        ChannelListener(LXAbstractChannel channel) {
            ...
            if (channel instanceof LXChannel) {
                ...
                c.controlSurfaceFocusIndex.setValue(
                        focusedPatternIndex < CLIP_LAUNCH_ROWS ? 0 : (focusedPatternIndex - CLIP_LAUNCH_ROWS + 1));
            }
    ...
    public final BooleanParameter masterFaderEnabled = new BooleanParameter("Master Fader", true)
            .setDescription("Whether the master fader for output brightness is enabled");

    public final BooleanParameter crossfaderEnabled =
            new BooleanParameter("Crossfader", true).setDescription("Whether the A/B crossfader is enabled");

    public final BooleanParameter performanceLock = new BooleanParameter("Performance Lock", false)
    ...
    private void sendClip(int channelIndex, LXAbstractChannel channel, int clipIndex, LXClip clip) {
        ...
        if (this.gridMode != GridMode.CLIP || channelIndex >= CLIP_LAUNCH_COLUMNS || slotIndex >= CLIP_LAUNCH_ROWS) {
            return;
        }
        int color = LED_OFF;
    ...
    public void controlChangeReceived(MidiControlChange cc) {
        ...
        switch (number) {
            case TEMPO:
                if (this.gridMode == GridMode.PALETTE) {
                    if (this.rainbowMode) {
                        this.rainbowColumnOffset = (this.rainbowColumnOffset + cc.getRelative()) % RAINBOW_GRID_COLUMNS;
                        this.focusColor = null;
                        this.colorClipboard = null;
                    } else {
