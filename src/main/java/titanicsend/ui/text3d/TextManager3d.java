...
public class TextManager3d {
  ...
  /**
   * Initialize a TextManager3d object with a font atlas file path TODO - only a single font can
   * currently be associated with a text manager/renderer TODO - Are we ever going to need more than
   * this?
   *
  ...
  /**
   * sets the font scale multiplier. Default is 10000, which yields a font that, when rendered in
   * 3D, appears to be somewhere between 10 and 16 points. (on the TE model, it looks like it'd be a
   * bit over a foot high.) Does not retroactively change the size of existing labels, so to change
  ...
  /**
   * Sets the font color for all new labels. Does not retroactively change the color of existing
   * labels.
   *
  ...
  /**
   * Sets the font background color for all new labels. Does not retroactively change f existing
   * labels.
   *
   * @param color packed LX color
   */
