...
public class UIUtils {
     ...
     */
    public static void buildMftStyleDeviceControls(UI ui, UIDevice uiDevice, LXDeviceComponent device) {
        int ki = 0;
        int col = 0;
        for (LXNormalizedParameter param : device.getRemoteControls()) {
            ...
            if (param instanceof TEColorParameter.TEColorOffsetParameter) {
                new UITEColorControl(x, y, (TEColorParameter) param.getParentParameter()).addToContainer(uiDevice);
