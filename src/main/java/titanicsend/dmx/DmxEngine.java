...
public class DmxEngine implements LXLoopTask {
    ...
    private void runDmxMixer() {
            ...
            if (!isSubChannel) {
                ...
                if (blendStack != null && channel.enabled.isOn()) {
                    ...
                    if (transitionProgress > 0) {
                        ...
                        blendStack.blend(
                                channel.blendMode.getObject(), getDmxBuffersByChannel(channel), alpha, dmxWholeModel);
        ...
        if (leftContent && rightContent) {
            ...
            this.blendStackMain.blend(
                    this.addBlend, this.blendStackLeft, Math.min(1, 2. * (1 - crossfadeValue)), dmxWholeModel);
    ...
    private DmxBuffer[] getDmxBuffersByChannel(LXAbstractChannel channel) {
        if (channel instanceof LXGroup) {
            ...
            return this.dmxBufferByLXBuffer
                    .get(this.lxBuffersByChannel.get(channel).get(0))
                    .getArray();
        }
    }
    ...
    private DmxBuffer[] getRenderBuffersByChannel(LXAbstractChannel channel) {
        return this.dmxBufferByLXBuffer
                .get(this.lxBuffersByChannel.get(channel).get(1))
                .getArray();
    }
